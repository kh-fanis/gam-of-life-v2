...
describe GenerationLog do
  ...
  it 'should add generation to stack' do
    ...
    expect(generation_log.add(Generation.new(GridGenerator.generate!(grid)))).to eql true
    expect(generation_log.add(Generation.new(GridGenerator.generate!(grid)))).to eql false
  end

  it 'should return last number of generations' do
    generation_log.add(Generation.new(grid))
    generation_log.add(Generation.new(GridGenerator.generate!(grid)))
    expect(generation_log.count).to eql 2
  end
end